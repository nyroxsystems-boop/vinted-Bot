// Blackruby Crosslister — Leboncoin (FR) Content Script
console.log('[LEBONCOIN] Loaded:', window.location.href);
const PLATFORM = 'leboncoin';
let busy = false;
...
async function go(p) {
  ...
  try {
    await waitFor('input[name="subject"], input[placeholder*="titre" i], input[name*="title" i]');
    fill('input[name="subject"], input[placeholder*="titre" i], input[name*="title" i]', (d.title||'').slice(0,80));
    await dl(500);
    fill('textarea[name="body"], textarea[placeholder*="décri" i], textarea[name*="description" i]', d.description||'');
    await dl(500);
    fill('input[name="price"], input[placeholder*="prix" i], input[name*="price" i]', String(Math.round(d.price_eur||0)));
    await dl(500);
