// Blackruby Crosslister — Willhaben (AT) Content Script
console.log('[WILLHABEN] Loaded:', window.location.href);
const PLATFORM = 'willhaben';
let busy = false;
...
async function go(p) {
  ...
  try {
    await waitFor('input[name="description"], input[id*="title"], input[placeholder*="Titel" i]');
    fill('input[name="description"], input[id*="title"], input[placeholder*="Titel" i]', (d.title||'').slice(0,80));
    await dl(500);
    fill('textarea[name="body"], textarea[id*="body"], textarea[placeholder*="Beschreibung" i]', d.description||'');
    await dl(500);
    fill('input[name="price"], input[id*="price"], input[placeholder*="Preis" i]', String(Math.round(d.price_eur||0)));
    await dl(500);
