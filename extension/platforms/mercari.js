/**
 * Blackruby Crosslister — Mercari Content Script
 */
console.log('[MERCARI] Loaded:', window.location.href);
...
async function go(p) {
  ...
  try {
    await waitFor('input[name="name"], input[data-testid*="name"], input[placeholder*="What are you selling" i]');
    fill('input[name="name"], input[data-testid*="name"], input[placeholder*="selling" i]', (d.title||'').slice(0,80));
    await dl(500);
    fill('textarea[name="description"], textarea[data-testid*="description"]', (d.description||'').slice(0,1000));
    await dl(500);
    fill('input[name="price"], input[data-testid*="price"]', String(Math.round((d.price_eur||0)*1.08)));
    await dl(500);
    if(d.brand){fill('input[name*="brand" i], input[placeholder*="Brand" i]',d.brand);await dl(800);const s=document.querySelector('[role="option"]');if(s)s.click();}
    const fi=document.querySelector('input[type="file"]');
    if(fi&&d.image_urls?.length){await upImg(fi,d.image_urls,12);await dl(2000);}
