/**
 * Blackruby Crosslister — Wallapop Content Script
 */
console.log('[WALLAPOP] Loaded:', window.location.href);
...
async function go(p) {
  ...
  try {
    await waitFor('input[formcontrolname="title"], input[placeholder*="título" i], input[name*="title" i]');
    fill('input[formcontrolname="title"], input[placeholder*="título" i], input[name*="title" i]', (d.title||'').slice(0,50));
    await dl(500);
    fill('textarea[formcontrolname="description"], textarea[placeholder*="Describe" i]', d.description||'');
    await dl(500);
    fill('input[formcontrolname="salePrice"], input[name*="price" i], input[placeholder*="precio" i]', String(Math.round(d.price_eur||0)));
    await dl(500);
