// Blackruby Crosslister — Vestiaire Collective Content Script
console.log('[VESTIAIRE] Loaded:', window.location.href);
...
async function go(p) {
  ...
  try {
    await waitFor('input[name*="title" i], input[placeholder*="title" i]');
    fill('input[name*="title" i], input[placeholder*="title" i]', (d.title||'').slice(0,80));
    await dl(500);
    fill('textarea[name*="description" i], textarea[placeholder*="description" i]', d.description||'');
    await dl(500);
    fill('input[name*="price" i], input[placeholder*="price" i]', String(Math.round(d.price_eur||0)));
    await dl(500);
    if(d.brand){fill('input[name*="brand" i], input[name*="designer" i], input[placeholder*="designer" i]',d.brand);await dl(1000);const s=document.querySelector('[role="option"]');if(s)s.click();}
    const fi=document.querySelector('input[type="file"]');
    if(fi&&d.image_urls?.length){await upImg(fi,d.image_urls,8);await dl(2000);}
