// Blackruby Crosslister — Subito (IT) Content Script
console.log('[SUBITO] Loaded:', window.location.href);
const PLATFORM = 'subito';
let busy = false;
...
async function go(p) {
  ...
  try {
    await waitFor('input[name="subject"], input[placeholder*="titolo" i], input[name*="title" i]');
    fill('input[name="subject"], input[placeholder*="titolo" i], input[name*="title" i]', (d.title||'').slice(0,80));
    await dl(500);
    fill('textarea[name="body"], textarea[placeholder*="descri" i], textarea[name*="description" i]', d.description||'');
    await dl(500);
    fill('input[name="price"], input[placeholder*="prezzo" i], input[name*="price" i]', String(Math.round(d.price_eur||0)));
    await dl(500);
