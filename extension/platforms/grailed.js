/**
 * Blackruby Crosslister — Grailed Content Script
 */
console.log('[GRAILED] Loaded:', window.location.href);
...
async function go(p) {
  ...
  try {
    ...
    await dl(500);
    fill('input[name*="price" i], input[placeholder*="price" i]', String(Math.round((d.price_eur||0)*1.08)));
    await dl(500);
    if(d.brand) { fill('input[name*="designer" i], input[name*="brand" i], input[placeholder*="designer" i]', d.brand); await dl(800); const s=document.querySelector('[role="option"]'); if(s)s.click(); }
    const fi=document.querySelector('input[type="file"]');
    if(fi&&d.image_urls?.length) { await upImg(fi,d.image_urls,6); await dl(2000); }
