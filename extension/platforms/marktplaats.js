// Blackruby Crosslister — Marktplaats (NL) Content Script
console.log('[MARKTPLAATS] Loaded:', window.location.href);
...
async function go(p) {
  ...
  try {
    await waitFor('input[name="title"], input[id*="title"], input[placeholder*="titel" i]');
    fill('input[name="title"], input[id*="title"], input[placeholder*="titel" i]', (d.title||'').slice(0,80));
    await dl(500);
    fill('textarea[name="description"], textarea[id*="description"], textarea[placeholder*="beschrijving" i]', d.description||'');
    await dl(500);
    fill('input[name="price"], input[id*="price"], input[placeholder*="prijs" i]', String(Math.round(d.price_eur||0)));
    await dl(500);
