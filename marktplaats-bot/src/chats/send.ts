...
const log = createLogger('marktplaats-chats-send');
...
/** Send a free-text reply into a Marktplaats conversation. */
export async function sendMarktplaatsMessage(
  ctx: BrowserContext,
  ...
  try {
    ...
    log.info('Marktplaats reply sent', { conversationId, chars: body.length });
    return { ok: true };
  } catch (err) {
