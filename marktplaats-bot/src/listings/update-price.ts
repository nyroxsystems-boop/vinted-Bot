// ──────────────────────────────────────────────────────────────────────────────
// Marktplaats Listing-Price-Update
//
// Opens the edit-form for an existing Marktplaats listing and overwrites the price.
// Used by the orchestrator's repricer to nudge listings without re-creating.
// NOTE: Marktplaats prices are USD. We convert EUR → USD with EUR_USD_RATE (env).
// ──────────────────────────────────────────────────────────────────────────────
...
const log = createLogger('marktplaats-update-price');
const BASE_URL = process.env.MARKTPLAATS_BASE_URL ?? 'https://www.marktplaats.com';
...
/**
 * Updates the price of a Marktplaats listing. The `newPriceEur` is converted to
 * USD with `EUR_USD_RATE` (default 1.07). External-ID is Marktplaats's `m12345…`
 * style id (the segment after /item/).
 */
export async function updateMarktplaatsListingPrice(
  ctx: BrowserContext,
  externalId: string,
  newPriceEur: number,
  ...
  try {
    // Marktplaats edit URL — verified pattern. The product page redirects sellers
    ...
    log.info('Marktplaats price updated', { externalId, newPriceEur, newPriceUsd });
    return { ok: true };
  } catch (err) {
    const error = err instanceof Error ? err.message : String(err);
    log.warn('updateMarktplaatsListingPrice failed', { externalId, error });
