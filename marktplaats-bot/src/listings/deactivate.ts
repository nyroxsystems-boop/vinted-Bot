// Marktplaats Listing deaktivieren — über "Keep private" (versteckt aber nicht löscht)
...
const log = createLogger('marktplaats-listing-deactivate');

const BASE = 'https://www.marktplaats.com';

export async function deactivateMarktplaatsListing(
  page: Page,
  ...
  try {
    ...
    if ((opts.mode ?? 'hide') === 'hide') {
      try {
        ...
        await page.waitForTimeout(500);
        log.info('Marktplaats listing hidden', { externalIdOrUrl });
        return { ok: true };
      } catch {
    ...
    try {
      ...
      await page.waitForTimeout(500);
      log.info('Marktplaats listing deleted', { externalIdOrUrl });
      return { ok: true };
    } catch (err) {
