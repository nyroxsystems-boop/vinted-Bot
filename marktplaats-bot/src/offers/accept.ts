// ──────────────────────────────────────────────────────────────────────────────
// Marktplaats Offer — accept
//
// Marktplaats buyers can press "Make Offer". Seller (us) gets the offer in the
...
const log = createLogger('marktplaats-offer-accept');
...
/**
 * Accept a Marktplaats offer. `offerId` is the conversation id (Marktplaats uses the
 * chat thread as the offer container). We support either a raw conversation
 * id or a direct item id as a fallback.
 */
export async function acceptMarktplaatsOffer(
  ctx: BrowserContext,
  offerId: string,
): Promise<OfferResult> {
  ...
  try {
    ...
    log.info('Marktplaats offer accepted', { offerId });
    return { ok: true };
  } catch (err) {
