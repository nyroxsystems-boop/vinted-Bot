...
const log = createLogger('marktplaats-offer-decline');
...
export async function declineMarktplaatsOffer(
  ctx: BrowserContext,
  offerId: string,
): Promise<OfferResult> {
  ...
  try {
    ...
    log.info('Marktplaats offer declined', { offerId });
    return { ok: true };
  } catch (err) {
