// Willhaben Listing deaktivieren — über "Keep private" (versteckt aber nicht löscht)
...
const log = createLogger('willhaben-listing-deactivate');

const BASE = 'https://www.willhaben.com';

export async function deactivateWillhabenListing(
  page: Page,
  ...
  try {
    ...
    if ((opts.mode ?? 'hide') === 'hide') {
      try {
        ...
        await page.waitForTimeout(500);
        log.info('Willhaben listing hidden', { externalIdOrUrl });
        return { ok: true };
      } catch {
    ...
    try {
      ...
      await page.waitForTimeout(500);
      log.info('Willhaben listing deleted', { externalIdOrUrl });
      return { ok: true };
    } catch (err) {
