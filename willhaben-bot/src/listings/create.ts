// ──────────────────────────────────────────────────────────────────────────────
// Willhaben Listing-Create Flow
//
// Willhaben (US) — englischer Markt. Wir mappen ListingDraft → Willhaben-Form:
//   - title (max 80 chars)
...
const log = createLogger('willhaben-listing-create');

const HOME_URL = 'https://www.willhaben.com';
const SELL_URL = 'https://www.willhaben.com/sell/';
const USD_RATE = Number(process.env.EUR_USD_RATE ?? 1.07); // konservativ

// ── Mappings ─────────────────────────────────────────────────────────────────

function buildTitle(draft: ListingDraft): string {
  // Willhaben titles bevorzugen "Brand + Item Type + Color + Size".
...
function translateCondition(de: string): string {
  // Willhaben: New | Like new | Good | Fair | Poor
  const map: Record<string, string> = {
    'Neu, mit Etikett': 'New',
    'Neu': 'New',
    'Sehr gut': 'Like new',
...
function buildDescription(draft: ListingDraft): string {
  ...
  // ergänzen aber englische Spec-Zeilen damit Willhaben-Suche es findet.
  const lines: string[] = [];
...
async function pickCategory(page: Page, vintedCat: string, warnings: string[]): Promise<void> {
  const target = mapCategory('willhaben', vintedCat);
...
export async function createWillhabenListing(
  page: Page,
  draft: ListingDraft,
  ...
  try {
    ...
    if (page.url() === HOME_URL || page.url().endsWith('willhaben.com/')) {
