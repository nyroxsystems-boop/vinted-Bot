...
const log = createLogger('willhaben-offer-decline');
...
export async function declineWillhabenOffer(
  ctx: BrowserContext,
  offerId: string,
): Promise<OfferResult> {
  ...
  try {
    ...
    log.info('Willhaben offer declined', { offerId });
    return { ok: true };
  } catch (err) {
