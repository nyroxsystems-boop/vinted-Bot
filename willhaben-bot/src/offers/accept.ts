// ──────────────────────────────────────────────────────────────────────────────
// Willhaben Offer — accept
//
// Willhaben buyers can press "Make Offer". Seller (us) gets the offer in the
...
const log = createLogger('willhaben-offer-accept');
...
/**
 * Accept a Willhaben offer. `offerId` is the conversation id (Willhaben uses the
 * chat thread as the offer container). We support either a raw conversation
 * id or a direct item id as a fallback.
 */
export async function acceptWillhabenOffer(
  ctx: BrowserContext,
  offerId: string,
): Promise<OfferResult> {
  ...
  try {
    ...
    log.info('Willhaben offer accepted', { offerId });
    return { ok: true };
  } catch (err) {
