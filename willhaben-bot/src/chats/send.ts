...
const log = createLogger('willhaben-chats-send');
...
/** Send a free-text reply into a Willhaben conversation. */
export async function sendWillhabenMessage(
  ctx: BrowserContext,
  ...
  try {
    ...
    log.info('Willhaben reply sent', { conversationId, chars: body.length });
    return { ok: true };
  } catch (err) {
