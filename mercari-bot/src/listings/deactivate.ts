// Mercari Listing deaktivieren — über "Keep private" (versteckt aber nicht löscht)
...
const log = createLogger('mercari-listing-deactivate');

const BASE = 'https://www.mercari.com';

export async function deactivateMercariListing(
  page: Page,
  ...
  try {
    ...
    if ((opts.mode ?? 'hide') === 'hide') {
      try {
        ...
        await page.waitForTimeout(500);
        log.info('Mercari listing hidden', { externalIdOrUrl });
        return { ok: true };
      } catch {
    ...
    try {
      ...
      await page.waitForTimeout(500);
      log.info('Mercari listing deleted', { externalIdOrUrl });
      return { ok: true };
    } catch (err) {
