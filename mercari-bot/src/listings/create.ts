// ──────────────────────────────────────────────────────────────────────────────
// Mercari Listing-Create Flow
//
// Mercari (US) — englischer Markt. Wir mappen ListingDraft → Mercari-Form:
//   - title (max 80 chars)
...
const log = createLogger('mercari-listing-create');

const HOME_URL = 'https://www.mercari.com';
const SELL_URL = 'https://www.mercari.com/sell/';
const USD_RATE = Number(process.env.EUR_USD_RATE ?? 1.07); // konservativ

// ── Mappings ─────────────────────────────────────────────────────────────────

function buildTitle(draft: ListingDraft): string {
  // Mercari titles bevorzugen "Brand + Item Type + Color + Size".
...
function translateCondition(de: string): string {
  // Mercari: New | Like new | Good | Fair | Poor
  const map: Record<string, string> = {
    'Neu, mit Etikett': 'New',
    'Neu': 'New',
    'Sehr gut': 'Like new',
...
function buildDescription(draft: ListingDraft): string {
  ...
  // ergänzen aber englische Spec-Zeilen damit Mercari-Suche es findet.
  const lines: string[] = [];
...
async function pickCategory(page: Page, vintedCat: string, warnings: string[]): Promise<void> {
  const target = mapCategory('mercari', vintedCat);
...
export async function createMercariListing(
  ...
  try {
    ...
    if (page.url() === HOME_URL || page.url().endsWith('mercari.com/')) {
