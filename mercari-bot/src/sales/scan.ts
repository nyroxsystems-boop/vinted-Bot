// ──────────────────────────────────────────────────────────────────────────────
// Mercari Sold-Item Detection
//
// Visits the seller's sold-listings tab and scrapes external IDs. Matching
// rows in `marketplace_listings` (marketplace='mercari', status='active')
...
const log = createLogger('mercari-sales-scan');
const BASE_URL = process.env.MERCARI_BASE_URL ?? 'https://www.mercari.com';

// TODO: validate sold-page URL. Mercari sellers see sold items on their
...
async function scrapeSoldItems(page: Page): Promise<SoldItem[]> {
  ...
  const items = await page.evaluate(() => {
    ...
    for (const a of anchors) {
      ...
      seen.add(m[1]);
      out.push({ id: m[1], url: href.startsWith('http') ? href : `https://www.mercari.com${href}` });
    }
    return out;
  });
...
/**
 * Scan Mercari sold items. For every item found, flips matching
 * `marketplace_listings` row (marketplace='mercari', account_id=N, status='active')
 * to 'sold'. Idempotent.
 */
export async function scanMercariSold(
  accountId: number,
  ctx: BrowserContext,
  ...
  try {
    ...
    const find = db.prepare(`
      SELECT id, folder_num, status FROM marketplace_listings
       WHERE marketplace = 'mercari' AND account_id = ?
         AND external_id = ?
         AND status = 'active'
    `);
    ...
    for (const it of items) {
      ...
      flip.run(row.id);
      stats.newlySold++;
      log.info('Mercari marked sold', { externalId: it.externalId, folderNum: row.folder_num });
    }
  } finally {
