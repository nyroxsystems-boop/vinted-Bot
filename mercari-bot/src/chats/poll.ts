// ──────────────────────────────────────────────────────────────────────────────
// Mercari Inbox Poller
//
// Walks Mercari's `/mypage/messages/` inbox and returns a structured list of
// conversations + recent messages. Stateless: no DB writes for chats — the
// orchestrator decides how to persist (no `mercari_chats` table exists yet,
...
const log = createLogger('mercari-chats-poll');
const BASE_URL = process.env.MERCARI_BASE_URL ?? 'https://www.mercari.com';

// Mercari inbox URL — TODO: validate. Alternate paths: /inbox/, /mypage/inbox/.
...
function extractConversationId(href: string): string {
  // Mercari conversation URLs look like /messages/xyz123 or /inbox/xyz123.
...
async function scrapeConversation(
  ...
  const messages = await page.evaluate(() => {
    ...
    return nodes.map((n) => {
      ...
      // Mercari offers: "offered $X" / "Buyer made an offer"
...
/**
 * Poll the Mercari inbox for `accountId`. Returns a structured payload —
 * persistence is the orchestrator's responsibility.
 *
 * Bot is stateless w.r.t. chats — no `mercari_chats` table exists. Caller
 * receives the full list and decides how to fan out (reply-autopilot,
 * notifications, etc.).
 */
export async function pollMercariInbox(
  accountId: number,
  ctx: BrowserContext,
): Promise<PollStats> {
