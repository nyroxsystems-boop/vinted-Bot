// ──────────────────────────────────────────────────────────────────────────────
// Mercari Offer — accept
//
// Mercari buyers can press "Make Offer". Seller (us) gets the offer in the
...
const log = createLogger('mercari-offer-accept');
...
/**
 * Accept a Mercari offer. `offerId` is the conversation id (Mercari uses the
 * chat thread as the offer container). We support either a raw conversation
 * id or a direct item id as a fallback.
 */
export async function acceptMercariOffer(
  ctx: BrowserContext,
  offerId: string,
): Promise<OfferResult> {
  ...
  try {
    ...
    log.info('Mercari offer accepted', { offerId });
    return { ok: true };
  } catch (err) {
