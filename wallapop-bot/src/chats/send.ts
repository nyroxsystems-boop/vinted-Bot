...
const log = createLogger('wp-chats-send');
const BASE_URL = process.env.WALLAPOP_BASE_URL ?? 'https://es.wallapop.com';

function convUrl(conversationId: string): string {
  // TODO: validate URL pattern. Could also be `/app/chat?id=<id>`.
  return `${BASE_URL}/chat/${conversationId}`;
}
...
/** Send a free-text reply into a Wallapop conversation. */
export async function sendWallapopMessage(
  ...
  try {
    ...
    log.info('Wallapop reply sent', { conversationId, chars: body.length });
    return { ok: true };
  } catch (err) {
