// ──────────────────────────────────────────────────────────────────────────────
// Leboncoin Offer — accept
//
// Leboncoin buyers can press "Make Offer". Seller (us) gets the offer in the
...
const log = createLogger('leboncoin-offer-accept');
...
/**
 * Accept a Leboncoin offer. `offerId` is the conversation id (Leboncoin uses the
 * chat thread as the offer container). We support either a raw conversation
 * id or a direct item id as a fallback.
 */
export async function acceptLeboncoinOffer(
  ctx: BrowserContext,
  offerId: string,
): Promise<OfferResult> {
  ...
  try {
    ...
    log.info('Leboncoin offer accepted', { offerId });
    return { ok: true };
  } catch (err) {
