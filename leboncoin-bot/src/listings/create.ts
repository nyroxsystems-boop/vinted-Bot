// ──────────────────────────────────────────────────────────────────────────────
// Leboncoin Listing-Create Flow
//
// Leboncoin (US) — englischer Markt. Wir mappen ListingDraft → Leboncoin-Form:
//   - title (max 80 chars)
...
const log = createLogger('leboncoin-listing-create');

const HOME_URL = 'https://www.leboncoin.com';
const SELL_URL = 'https://www.leboncoin.com/sell/';
const USD_RATE = Number(process.env.EUR_USD_RATE ?? 1.07); // konservativ

// ── Mappings ─────────────────────────────────────────────────────────────────

function buildTitle(draft: ListingDraft): string {
  // Leboncoin titles bevorzugen "Brand + Item Type + Color + Size".
...
function translateCondition(de: string): string {
  // Leboncoin: New | Like new | Good | Fair | Poor
  const map: Record<string, string> = {
    'Neu, mit Etikett': 'New',
    'Neu': 'New',
    'Sehr gut': 'Like new',
...
function buildDescription(draft: ListingDraft): string {
  ...
  // ergänzen aber englische Spec-Zeilen damit Leboncoin-Suche es findet.
  const lines: string[] = [];
...
async function pickCategory(page: Page, vintedCat: string, warnings: string[]): Promise<void> {
  const target = mapCategory('leboncoin', vintedCat);
...
export async function createLeboncoinListing(
  page: Page,
  draft: ListingDraft,
  ...
  try {
    ...
    if (page.url() === HOME_URL || page.url().endsWith('leboncoin.com/')) {
