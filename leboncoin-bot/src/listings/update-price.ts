// ──────────────────────────────────────────────────────────────────────────────
// Leboncoin Listing-Price-Update
//
// Opens the edit-form for an existing Leboncoin listing and overwrites the price.
// Used by the orchestrator's repricer to nudge listings without re-creating.
// NOTE: Leboncoin prices are USD. We convert EUR → USD with EUR_USD_RATE (env).
// ──────────────────────────────────────────────────────────────────────────────
...
const log = createLogger('leboncoin-update-price');
...
/**
 * Updates the price of a Leboncoin listing. The `newPriceEur` is converted to
 * USD with `EUR_USD_RATE` (default 1.07). External-ID is Leboncoin's `m12345…`
 * style id (the segment after /item/).
 */
export async function updateLeboncoinListingPrice(
  ctx: BrowserContext,
  externalId: string,
  newPriceEur: number,
  ...
  try {
    // Leboncoin edit URL — verified pattern. The product page redirects sellers
    ...
    log.info('Leboncoin price updated', { externalId, newPriceEur, newPriceUsd });
    return { ok: true };
  } catch (err) {
    const error = err instanceof Error ? err.message : String(err);
    log.warn('updateLeboncoinListingPrice failed', { externalId, error });
