// Leboncoin Listing deaktivieren — über "Keep private" (versteckt aber nicht löscht)
...
const log = createLogger('leboncoin-listing-deactivate');

const BASE = 'https://www.leboncoin.com';

export async function deactivateLeboncoinListing(
  page: Page,
  ...
  try {
    ...
    if ((opts.mode ?? 'hide') === 'hide') {
      try {
        ...
        await page.waitForTimeout(500);
        log.info('Leboncoin listing hidden', { externalIdOrUrl });
        return { ok: true };
      } catch {
    ...
    try {
      ...
      await page.waitForTimeout(500);
      log.info('Leboncoin listing deleted', { externalIdOrUrl });
      return { ok: true };
    } catch (err) {
