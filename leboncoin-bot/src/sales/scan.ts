// ──────────────────────────────────────────────────────────────────────────────
// Leboncoin Sold-Item Detection
//
// Visits the seller's sold-listings tab and scrapes external IDs. Matching
// rows in `marketplace_listings` (marketplace='leboncoin', status='active')
...
const log = createLogger('leboncoin-sales-scan');
const BASE_URL = process.env.LEBONCOIN_BASE_URL ?? 'https://www.leboncoin.com';

// TODO: validate sold-page URL. Leboncoin sellers see sold items on their
...
async function scrapeSoldItems(page: Page): Promise<SoldItem[]> {
  ...
  const items = await page.evaluate(() => {
    ...
    for (const a of anchors) {
      ...
      seen.add(m[1]);
      out.push({ id: m[1], url: href.startsWith('http') ? href : `https://www.leboncoin.com${href}` });
    }
    return out;
  });
...
/**
 * Scan Leboncoin sold items. For every item found, flips matching
 * `marketplace_listings` row (marketplace='leboncoin', account_id=N, status='active')
 * to 'sold'. Idempotent.
 */
export async function scanLeboncoinSold(
  accountId: number,
  ctx: BrowserContext,
  ...
  try {
    ...
    const find = db.prepare(`
      SELECT id, folder_num, status FROM marketplace_listings
       WHERE marketplace = 'leboncoin' AND account_id = ?
         AND external_id = ?
         AND status = 'active'
    `);
    ...
    for (const it of items) {
      ...
      flip.run(row.id);
      stats.newlySold++;
      log.info('Leboncoin marked sold', { externalId: it.externalId, folderNum: row.folder_num });
    }
  } finally {
