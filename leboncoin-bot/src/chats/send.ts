...
const log = createLogger('leboncoin-chats-send');
...
/** Send a free-text reply into a Leboncoin conversation. */
export async function sendLeboncoinMessage(
  ctx: BrowserContext,
  ...
  try {
    ...
    log.info('Leboncoin reply sent', { conversationId, chars: body.length });
    return { ok: true };
  } catch (err) {
