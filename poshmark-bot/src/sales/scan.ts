// ──────────────────────────────────────────────────────────────────────────────
// Poshmark Sold-Item Detection
//
// Visits the seller's sold-listings tab and scrapes external IDs. Matching
// rows in `marketplace_listings` (marketplace='poshmark', status='active')
...
const log = createLogger('poshmark-sales-scan');
const BASE_URL = process.env.POSHMARK_BASE_URL ?? 'https://www.poshmark.com';

// TODO: validate sold-page URL. Poshmark sellers see sold items on their
...
async function scrapeSoldItems(page: Page): Promise<SoldItem[]> {
  ...
  const items = await page.evaluate(() => {
    ...
    for (const a of anchors) {
      ...
      seen.add(m[1]);
      out.push({ id: m[1], url: href.startsWith('http') ? href : `https://www.poshmark.com${href}` });
    }
    return out;
  });
...
/**
 * Scan Poshmark sold items. For every item found, flips matching
 * `marketplace_listings` row (marketplace='poshmark', account_id=N, status='active')
 * to 'sold'. Idempotent.
 */
export async function scanPoshmarkSold(
  accountId: number,
  ctx: BrowserContext,
  ...
  try {
    ...
    const find = db.prepare(`
      SELECT id, folder_num, status FROM marketplace_listings
       WHERE marketplace = 'poshmark' AND account_id = ?
         AND external_id = ?
         AND status = 'active'
    `);
    ...
    for (const it of items) {
      ...
      flip.run(row.id);
      stats.newlySold++;
      log.info('Poshmark marked sold', { externalId: it.externalId, folderNum: row.folder_num });
    }
  } finally {
