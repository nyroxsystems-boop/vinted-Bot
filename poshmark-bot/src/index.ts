// ──────────────────────────────────────────────────────────────────────────────
// Poshmark-Bot HTTP Server :4719 — Scaffold mit funktionierendem
...
import { createPoshmarkListing } from './listings/create.js';
import { deactivatePoshmarkListing } from './listings/deactivate.js';
import { updatePoshmarkListingPrice } from './listings/update-price.js';
import { pollPoshmarkInbox } from './chats/poll.js';
import { sendPoshmarkMessage } from './chats/send.js';
import { acceptPoshmarkOffer } from './offers/accept.js';
import { declinePoshmarkOffer } from './offers/decline.js';
import { scanPoshmarkSold } from './sales/scan.js';

const log = createLogger('poshmark-bot');
const PORT = Number(process.env.POSHMARK_BOT_PORT ?? 4719);
const DATA_ROOT = process.env.POSHMARK_DATA_ROOT
...
export const poshmarkAdapter: MarketplaceAdapter = {
  meta: MARKETPLACE_REGISTRY.poshmark,
  ...
  async login(accountId) {
    ...
    try {
      const page = await ctx.newPage();
      await page.goto('https://www.poshmark.com/login/', { timeout: 30_000 });
  ...
  async publish(accountId: number, draft: ListingDraft) {
    ...
    try {
      ...
      const result = await createPoshmarkListing(page, draft);
  ...
  async deactivate(accountId: number, externalIdOrUrl: string) {
    ...
    try {
      ...
      const result = await deactivatePoshmarkListing(page, externalIdOrUrl, { mode: 'hide' });
      await page.close();
      return result;
    } finally {
      await ctx.close();
  ...
  async updatePrice(accountId: number, externalId: string, newPriceEur: number) {
    const ctx = await launch(accountId, true);
    try {
      return await updatePoshmarkListingPrice(ctx, externalId, newPriceEur);
    } finally {
      await ctx.close();
    }
  },
...
// ── Health + auth ────────────────────────────────────────────────────────────
app.get('/api/health', (_req, res) => res.json({ ok: true, marketplace: 'poshmark', port: PORT }));

app.get('/api/auth/status', async (req, res) => {
  try { res.json({ ok: await poshmarkAdapter.isAuthenticated(Number(req.query.account_id ?? 1)) }); }
  catch (e) { res.status(500).json({ error: String(e) }); }
});

app.post('/api/auth/login', async (req, res) => {
  try { res.json(await poshmarkAdapter.login(Number(req.body?.account_id ?? 1))); }
  catch (e) { res.status(500).json({ error: String(e) }); }
});

// ── Listings ─────────────────────────────────────────────────────────────────
app.post('/api/listings/publish', async (req, res) => {
  try { res.json(await poshmarkAdapter.publish(Number(req.body?.account_id ?? 1), req.body?.draft)); }
  catch (e) { res.status(500).json({ error: String(e) }); }
});
...
app.post('/api/listings/deactivate', async (req, res) => {
  ...
  try { res.json(await poshmarkAdapter.deactivate(Number(req.body?.account_id ?? 1), externalId)); }
...
app.post('/api/listings/update-price', async (req, res) => {
  ...
  try {
    res.json(await poshmarkAdapter.updatePrice!(
...
app.post('/api/chats/poll', async (req, res) => {
  ...
  try {
    ...
    const stats = await pollPoshmarkInbox(accountId, ctx);
...
app.post('/api/chats/:convId/send', async (req, res) => {
  ...
  try {
    ctx = await launch(accountId, true);
    const r = await sendPoshmarkMessage(ctx, req.params.convId, body);
    res.json(r);
  } catch (e) {
    res.status(500).json({ ok: false, error: e instanceof Error ? e.message : String(e) });
  } finally {
...
app.post('/api/offers/:id/accept', async (req, res) => {
  ...
  try {
    ctx = await launch(accountId, true);
    const r = await acceptPoshmarkOffer(ctx, req.params.id);
    res.json(r);
  } catch (e) {
    res.status(500).json({ ok: false, error: e instanceof Error ? e.message : String(e) });
  } finally {
...
app.post('/api/offers/:id/decline', async (req, res) => {
  ...
  try {
    ctx = await launch(accountId, true);
    const r = await declinePoshmarkOffer(ctx, req.params.id);
    res.json(r);
  } catch (e) {
    res.status(500).json({ ok: false, error: e instanceof Error ? e.message : String(e) });
  } finally {
...
app.post('/api/sold/poll', async (req, res) => {
  ...
  try {
    ctx = await launch(accountId, true);
    const stats = await scanPoshmarkSold(accountId, ctx);
    res.json({ ok: true, ...stats });
  } catch (e) {
    res.status(500).json({ ok: false, error: e instanceof Error ? e.message : String(e) });
  } finally {
...
app.listen(PORT, () => log.info(`Poshmark-Bot listening on :${PORT}`));
