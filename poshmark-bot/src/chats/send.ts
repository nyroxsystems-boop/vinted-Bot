...
const log = createLogger('poshmark-chats-send');
...
/** Send a free-text reply into a Poshmark conversation. */
export async function sendPoshmarkMessage(
  ctx: BrowserContext,
  ...
  try {
    ...
    log.info('Poshmark reply sent', { conversationId, chars: body.length });
    return { ok: true };
  } catch (err) {
