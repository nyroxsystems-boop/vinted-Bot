// ──────────────────────────────────────────────────────────────────────────────
// Poshmark Listing-Create Flow
//
// Poshmark (US) — englischer Markt. Wir mappen ListingDraft → Poshmark-Form:
//   - title (max 80 chars)
...
const log = createLogger('poshmark-listing-create');

const HOME_URL = 'https://www.poshmark.com';
const SELL_URL = 'https://www.poshmark.com/sell/';
const USD_RATE = Number(process.env.EUR_USD_RATE ?? 1.07); // konservativ

// ── Mappings ─────────────────────────────────────────────────────────────────

function buildTitle(draft: ListingDraft): string {
  // Poshmark titles bevorzugen "Brand + Item Type + Color + Size".
...
function translateCondition(de: string): string {
  // Poshmark: New | Like new | Good | Fair | Poor
  const map: Record<string, string> = {
    'Neu, mit Etikett': 'New',
    'Neu': 'New',
    'Sehr gut': 'Like new',
...
function buildDescription(draft: ListingDraft): string {
  ...
  // ergänzen aber englische Spec-Zeilen damit Poshmark-Suche es findet.
  const lines: string[] = [];
...
async function pickCategory(page: Page, vintedCat: string, warnings: string[]): Promise<void> {
  const target = mapCategory('poshmark', vintedCat);
...
export async function createPoshmarkListing(
  page: Page,
  draft: ListingDraft,
  ...
  try {
    ...
    if (page.url() === HOME_URL || page.url().endsWith('poshmark.com/')) {
